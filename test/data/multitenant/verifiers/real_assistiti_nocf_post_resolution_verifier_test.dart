  ...
  group('RealAssistitiNoCfPostResolutionVerifier', () {
    ...
    test('rejects non-manual root status even when nested status is manual', () {
      final RealAssistitiNoCfPostResolutionVerificationItem item =
        ...
        targetExists: true,
        targetData: _targetPayload(
          identityResolutionStatus: 'resolved_auto',
          identityResolution: const <String, dynamic>{'status': 'resolved_manual'},
      ...
      expect(item.verified, isFalse);
      expect(item.identityResolutionStatus, 'resolved_auto');
      expect(item.nestedIdentityResolutionStatus, 'resolved_manual');
