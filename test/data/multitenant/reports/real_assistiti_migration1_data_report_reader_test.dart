...
void main() {
  group('RealAssistitiMigration1DataReportReader', () {
    ...
    test('rejects CF assistito contamination inside nome or cognome', () {
      final Map<String, dynamic> payload = _validCfPayload();
      payload['nome'] = 'Mario RSSMRA80A01H501U';

      final RealAssistitiMigration1DataReportItem item =
          RealAssistitiMigration1DataReportReader.buildItemFromRawData(
        tenantId: 'tenant_a',
        documentId: 'RSSMRA80A01H501U',
        rawData: payload,
      );

      expect(item.isCf, isTrue);
      expect(item.verified, isFalse);
      expect(item.mismatchReasons, contains('target_identity_contains_cf_token'));
    });
...
Map<String, dynamic> _validCfPayload() {
  ...
  const String fullName = 'Rossi Mario';
  return <String, dynamic>{
    'assistitoId': cf,
    'cf': cf,
    'identityType': 'cf',
