...
function readMigration1CostAuditBudgets_() {
  ...
  var maxReads = Number(props.getProperty(PHBOX_M1_COST_MAX_READS_PROPERTY_) || PHBOX_M1_COST_DEFAULT_MAX_READS_);
  var maxWrites = Number(props.getProperty(PHBOX_M1_COST_MAX_WRITES_PROPERTY_) || PHBOX_M1_COST_DEFAULT_MAX_WRITES_);
  return { maxReads: isNaN(maxReads) ? PHBOX_M1_COST_DEFAULT_MAX_READS_ : Math.max(0, maxReads), maxWrites: isNaN(maxWrites) ? PHBOX_M1_COST_DEFAULT_MAX_WRITES_ : Math.max(0, maxWrites) };
}
...
function buildMigration1CostAuditRuntimeResult_(gateStage, e2eStatus, budgets) {
  ...
  var maxReads = Math.max(0, Number((budgets && budgets.maxReads) || PHBOX_M1_COST_DEFAULT_MAX_READS_));
  var maxWrites = Math.max(0, Number((budgets && budgets.maxWrites) || PHBOX_M1_COST_DEFAULT_MAX_WRITES_));
