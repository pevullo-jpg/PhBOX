var PHBOX_M2_VERIFY_VERSION_ = 'M2_VERIFY_v3';
...
function runMigration2PostWriteVerifyForLegacyPaths_(cfg, dashStatus, legacyPaths) {
  ...
  var pathPlan = prevalidateMigration2PostWriteVerifySamplePathPlan_(tenantId, legacyPaths);
  var boundedPaths = pathPlan.boundedPaths;
  var normalizedLegacyPaths = pathPlan.normalizedLegacyPaths;
  var items = [];
  var reads = 0;
  ...
  normalizedLegacyPaths.forEach(function (normalizedLegacy) {
    var targetPath = buildMigration2PostWriteVerifyTargetPath_(tenantId, normalizedLegacy);
    targetPathBuilt = true;

    var legacyDoc = readMigration2PostWriteVerifyDocument_(cfg, normalizedLegacy.pathParts);
    var targetDoc = readMigration2PostWriteVerifyDocument_(cfg, targetPath.pathParts);
    reads += 2;
...
function prevalidateMigration2PostWriteVerifySamplePathPlan_(tenantId, legacyPaths) {
  var boundedPaths = normalizeMigration2PostWriteVerifySampleLegacyPaths_(legacyPaths);
  var normalizedLegacyPaths = boundedPaths.map(function (legacyPath) {
    return normalizeMigration2PostWriteVerifyLegacyPath_(legacyPath);
  });
  return {
    tenantId: String(tenantId || '').trim(),
    boundedPaths: boundedPaths,
    normalizedLegacyPaths: normalizedLegacyPaths
  };
}

function readMigration2PostWriteVerifyDocument_(cfg, pathParts) {
  if (cfg && typeof cfg.__m2VerifyGetDocument === 'function') {
    return cfg.__m2VerifyGetDocument(cfg, pathParts);
  }
  return getFirestoreDocumentByPath_(cfg, pathParts);
}
...
function runMigration2PostWriteVerifySelfTest_() {
  var cases = [
    ...
    },
    {
      ...
      expected: { matched: false, mismatchReason: 'signature_mismatch' }
    },
    {
      id: 'missing_target_detected',
      legacyDoc: { fiscalCode: 'RSSMRA80A01H501U', recipeCount: 2 },
      targetDoc: null,
      expected: { matched: false, mismatchReason: 'target_missing' }
    },
    {
    ...
    {
      id: 'mixed_valid_then_invalid_sample_rejected_before_reads',
      prevalidatePaths: ['patients/RSSMRA80A01H501U', 'tenants/farmacia_santa_venera/patients/RSSMRA80A01H501U'],
      expected: { ok: false, targetPathBuilt: false, firestoreReads: 0, errorContains: 'LEGACY_PATH_ALREADY_TARGET' }
    },
    {
...
function runMigration2PostWriteVerifySelfTestCase_(item) {
  try {
    ...
    if (item.prevalidatePaths) {
      var reads = 0;
      var cfg = {
        firestoreProjectId: 'phbox-test-project',
        __m2VerifyGetDocument: function () {
          reads++;
          return { ok: true };
        }
      };
      try {
        runMigration2PostWriteVerifyForLegacyPaths_(cfg, buildMigration2PostWriteVerifySyntheticDashStatus_({
        ...
        }), item.prevalidatePaths);
        return { ok: true, firestoreReads: reads, firestoreWrites: 0, targetPathBuilt: reads > 0 };
      } catch (prevalidationError) {
        return { ok: false, error: normalizeRuntimeErrorMessage_(prevalidationError), firestoreReads: reads, firestoreWrites: 0, targetPathBuilt: false };
      }
    }
...
function compareMigration2PostWriteVerifyExpected_(actual, expected) {
  ...
  if (Object.prototype.hasOwnProperty.call(expected, 'mismatchReason') && (actual.mismatchReasons || []).indexOf(expected.mismatchReason) === -1) mismatchReasons.push('expected_mismatch_reason_missing');
