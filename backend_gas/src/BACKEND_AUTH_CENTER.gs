...
function repairPhboxBackendMainTriggerFromAuthCenter() {
  ...
  try {
    ...
    writePhboxBackendHealthSnapshot_(healthInstallFailure);
    publishPhboxBackendAuthRuntimeSnapshot_(healthInstallFailure, { source: 'auth_center_repair_trigger_install_failure' });
