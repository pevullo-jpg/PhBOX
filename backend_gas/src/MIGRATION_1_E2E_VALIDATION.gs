...
function buildMigration1E2eValidationRuntimeResult_(gateStage, publishStatus, dualStatus, cutStatus) {
  ...
  var publishReads = Math.max(0, Number(publishStats.firestoreReads || 0));
  var dualReads = Math.max(0, Number(dualStats.firestoreReads || 0));
  var cutReads = Math.max(0, Number(cutStats.firestoreReads || 0));
  var dedupedCutReads = Math.max(0, cutReads - dualReads);
  var reads = publishReads + dualReads + dedupedCutReads;
...
function runMigration1E2eValidationSelfTest_() {
  var cases = [
    ...
    {
      id: 'cutover_true_reuses_dual_reads_without_double_count',
      input: buildMigration1E2eValidationRuntimeResult_(
        buildMigration1E2eTestGateStage_(true),
        buildMigration1E2eTestStage_(true, { reason: 'no_runtime_publish_attempted' }),
        buildMigration1E2eTestStage_(true, { samplePathsCompared: 2, firestoreReads: 4 }),
        buildMigration1E2eTestStage_(true, { cutover: true, dualVerified: true, samplePathsCompared: 2, firestoreReads: 4 })
      ),
