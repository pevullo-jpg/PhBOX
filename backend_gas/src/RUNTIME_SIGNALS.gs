...
function buildRuntimeSignalDoneWriteOverrides_(signal, result, nowIso) {
  ...
  var existingIdentityType = String(signal.identityType || '').trim().toLowerCase();
  var doneIdentityType = sanitizedIdentityType || existingIdentityType;
  var doneIdentityAnchor = String(sanitizedResult.identityAnchor || signal.identityAnchor || signal.targetIdentityAnchor || '').trim();
  var doneTargetFiscalCode = '';
  if (doneIdentityType === 'nocf') {
    doneTargetFiscalCode = '';
  } else if (doneIdentityType === 'cf') {
    doneTargetFiscalCode = normalizeCf_(sanitizedResult.cf || signal.targetFiscalCode || doneIdentityAnchor || '');
  } else {
    doneTargetFiscalCode = normalizeCf_(sanitizedResult.cf || signal.targetFiscalCode || '');
  }
  var overrides = {
    status: 'done',
    updatedAt: nowIso,
  ...
  var resolvedIdentity = resolveMigration1RuntimeSignalIdentity_(signal, overrides);
  overrides.result = buildRuntimeSignalDoneResultWithIdentity_(sanitizedResult, resolvedIdentity);
  return overrides;
}

function buildRuntimeSignalDoneResultWithIdentity_(sanitizedResult, identity) {
  sanitizedResult = sanitizedResult || {};
  identity = identity || {};
  var enriched = cloneRuntimePlainObject_(sanitizedResult);
  enriched.cf = String(identity.identityType || '').trim().toLowerCase() === 'cf' ? normalizeCf_(identity.targetFiscalCode || enriched.cf || '') : '';
  enriched.identityType = String(identity.identityType || '').trim().toLowerCase();
  enriched.identityAnchor = String(identity.identityAnchor || '').trim();
  enriched.identityAnchorCanonical = !!identity.identityAnchorCanonical;
  enriched.legacyNoCfCode = String(identity.legacyNoCfCode || '').trim();
  enriched.identityResolutionReasons = uniqueNonEmptyStrings_(identity.identityResolutionReasons || []);
  return enriched;
}
...
function buildMigration1DeletePdfRuntimeSignalIdentityOverrides_(signal, target, rawCf) {
  signal = signal || {};
  target = target || {};
  var explicitIdentityType = String(target.identityType || signal.identityType || '').trim().toLowerCase();
  var isNocfIdentity = explicitIdentityType === 'nocf';
  return {
    targetFiscalCode: isNocfIdentity ? '' : normalizeCf_(rawCf),
...
function processRuntimeDeletePdfSignal_(signal) {
  ...
  var nowIso = new Date().toISOString();
  var rawCf = normalizeCf_(signal.targetFiscalCode || target.patientFiscalCode || target.fiscalCode || target.patientCf);
  var identity = resolveMigration1RuntimeSignalIdentity_(signal, buildMigration1DeletePdfRuntimeSignalIdentityOverrides_(signal, target, rawCf));
  var cf = identity.targetFiscalCode;
