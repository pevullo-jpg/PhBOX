...
function markSignalDone_(signal, result) {
  ...
  var data = buildRuntimeSignalWriteData_(signal, {
    ...
    processedAt: nowIso,
    lastError: '',
    targetFiscalCode: sanitizedResult.cf || (signal && signal.targetFiscalCode) || '',
    identityType: sanitizedResult.identityType || (signal && signal.identityType) || '',
    identityAnchor: sanitizedResult.identityAnchor || (signal && signal.identityAnchor) || '',
    identityAnchorCanonical: sanitizedResult.identityAnchorCanonical,
    legacyNoCfCode: sanitizedResult.legacyNoCfCode || (signal && signal.legacyNoCfCode) || '',
    identityResolutionReasons: sanitizedResult.identityResolutionReasons || (signal && signal.identityResolutionReasons) || [],
