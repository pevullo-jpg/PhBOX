...
function processMigration4MaterializeRootPage_(cfg, api, cursor, tenantId, options) {
  ...
  return {
    ...
    work: work,
    writes: writes,
    stop: !!cursor.pageToken || ((options.currentWrites + writes.length) >= options.maxWrites)
  };
}
...
function processMigration4MaterializePatientSubcollectionPage_(cfg, api, cursor, tenantId, options) {
  ...
  return {
    ...
    work: work,
    writes: writes,
    stop: !!cursor.subcollectionPageToken || ((options.currentWrites + writes.length) >= options.maxWrites)
  };
}
...
function buildMigration4MaterializeWritesForDocs_(cfg, api, tenantId, documents, options) {
  ...
  for (var i = 0; i < documents.length; i++) {
    if ((options.currentWrites + writes.length) >= options.maxWrites) break;
