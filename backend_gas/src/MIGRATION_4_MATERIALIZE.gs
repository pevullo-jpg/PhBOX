...
function resolveMigration4MaterializeTenant_(api, props) {
  ...
  if (docs.length === 1) {
    ...
    if (tenantId !== PHBOX_M4_MATERIALIZE_APPROVED_TENANT_ID_) {
      throw new Error('M4_MATERIALIZE_TENANT_NOT_APPROVED: tenant registry Firestore diverso dal tenant approvato farmacia-santa-venera-8xnoc. Nessuna write target eseguita.');
    }
  ...
  throw new Error('M4_MATERIALIZE_TENANT_REGISTRY_EMPTY: nessun tenant nel registry Firestore. Script Properties non autorizzate come fallback. Nessuna write target eseguita.');
}
