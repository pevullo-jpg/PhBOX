var PHBOX_M2_ROUTE_VERSION_ = 'M2_ROUTE_v1';
...
function runMigration2RouteContractRuntimeStatus_() {
  ...
  try {
    if (typeof runMigration2LockRuntimeStatus_ !== 'function') {
      throw new Error('M2_ROUTE_LOCK_MISSING: funzione runMigration2LockRuntimeStatus_ non disponibile. Routing M2 non verificabile.');
    }
    lockStatus = runMigration2LockRuntimeStatus_();
  } catch (e) {
  ...
  var cases = [
    {
      ...
      result: buildMigration2RouteContractResult_({
        lockStatus: buildMigration2RouteSyntheticLockStatus_({}),
    ...
    {
      id: 'm2_lock_not_ok_blocks_route',
      result: buildMigration2RouteContractResult_({
        lockStatus: buildMigration2RouteSyntheticLockStatus_({ ok: false, reason: 'm2_lock_violation', violations: ['m2_lock_not_ready'] }),
        gateStatus: buildMigration2RouteSyntheticGateStatus_({}),
        routeMode: PHBOX_M2_ROUTE_MODE_LEGACY_
      }),
      expectedOk: false,
      expectedDecision: 'blocked',
      expectedViolation: 'm2_lock_not_ok'
    },
    {
...
function buildMigration2RouteContractResult_(data) {
  data = data || {};
  var lockStats = (data.lockStatus && data.lockStatus.stats) || {};
  ...
  if (!data.lockStatus || !data.lockStatus.stats) violations.push('m2_lock_status_missing');
  if (data.lockStatus && data.lockStatus.ok === false) violations.push('m2_lock_not_ok');
  if (Number(lockStats.firestoreReads || 0) !== 0) violations.push('m2_lock_reads_not_zero');
  if (Number(lockStats.firestoreWrites || 0) !== 0) violations.push('m2_lock_writes_not_zero');
  if (lockStats.publishToTarget || lockStats.publishFromTarget) violations.push('m2_lock_publish_detected');
  if (lockStats.targetPathBuilt) violations.push('m2_lock_target_path_built');
  if (lockStats.cutover) violations.push('m2_lock_cutover_detected');
  if (lockStats.lifecycleTouched) violations.push('m2_lock_lifecycle_touched');
  ...
  var decision = resolveMigration2RoutingDecision_(routeMode, lockStats, gateStats, violations);
  ...
  var stats = {
    ...
    lockOk: !!(data.lockStatus && data.lockStatus.ok),
...
function resolveMigration2RoutingDecision_(routeMode, lockStats, gateStats, baseViolations) {
  var violations = [];
...
function buildMigration2RouteSyntheticLockStatus_(overrides) {
  overrides = overrides || {};
  var stats = {
    ok: Object.prototype.hasOwnProperty.call(overrides, 'ok') ? !!overrides.ok : true,
    reason: String(overrides.reason || 'm2_lock_ready'),
    lockVersion: 'M2_LOCK_v3',
...
function formatMigration2RouteContractRuntimeFeedback_(result) {
  ...
  lines.push('lockOk=' + String(!!stats.lockOk));
