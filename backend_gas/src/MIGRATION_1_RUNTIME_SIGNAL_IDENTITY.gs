...
function runMigration1RuntimeSignalIdentitySelfTest_() {
  ...
  var items = cases.map(function (item) {
    var actual = item.doneResult
      ? buildRuntimeSignalWriteData_(item.input, buildMigration1RuntimeSignalIdentityDoneWriteTestOverrides_(item.doneResult))
      : (item.resultInput
    ...
    var expected = item.expected || item.expectedWriteData || item.expectedSanitized || item.expectedDoneWriteData || {};
...
function buildMigration1RuntimeSignalIdentityDoneWriteTestOverrides_(doneResult) {
  var sanitized = sanitizeRuntimeSignalResult_(doneResult || {});
  return {
    status: 'done',
    targetFiscalCode: sanitized.cf,
    identityType: sanitized.identityType,
    identityAnchor: sanitized.identityAnchor,
    identityAnchorCanonical: sanitized.identityAnchorCanonical,
    legacyNoCfCode: sanitized.legacyNoCfCode,
    identityResolutionReasons: sanitized.identityResolutionReasons,
    result: sanitized
  };
}
