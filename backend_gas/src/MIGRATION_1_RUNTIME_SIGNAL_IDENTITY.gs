...
function resolveMigration1RuntimeSignalIdentity_(signal, overrides) {
  ...
  var requestedIdentityType = String(overrides.identityType !== undefined ? overrides.identityType : (signal.identityType || '')).trim().toLowerCase();
  var requestedIdentityAnchor = String(overrides.identityAnchor || signal.identityAnchor || signal.targetIdentityAnchor || '').trim();
  var rawTargetFiscalCode = hasTargetFiscalCodeOverride ? overrides.targetFiscalCode : (signal.targetFiscalCode || '');
  if (!hasTargetFiscalCodeOverride && requestedIdentityType === PHBOX_M1_IDRES_IDENTITY_TYPE_NOCF_) {
    rawTargetFiscalCode = '';
  }
  if (!hasTargetFiscalCodeOverride && requestedIdentityType === PHBOX_M1_IDRES_IDENTITY_TYPE_CF_ && !normalizeCf_(rawTargetFiscalCode)) {
    rawTargetFiscalCode = requestedIdentityAnchor;
  }
  var legacyTargetCf = normalizeCf_(rawTargetFiscalCode);
  var identity = resolveMigration1BackendIdentity_({
    ...
    identityAnchor: requestedIdentityAnchor,
...
function runMigration1RuntimeSignalIdentitySelfTest_() {
  var cases = [
    ...
    {
      id: 'delete_pdf_nocf_target_with_stale_cf_ignores_cf_override',
      input: {
        domain: 'deletePdf',
        signalId: 'sig_delete_pdf_stale_cf_nocf_1',
        targetPath: 'drive_pdf_imports/file_1'
      },
      deletePdfTarget: {
        identityType: 'nocf',
        identityAnchor: 'NOCF_MANUAL_003',
        legacyNoCfCode: 'MANUAL_003',
        patientFiscalCode: 'RSSMRA80A01H501U',
        identityResolutionReasons: ['target_identity_verified']
      },
      expectedResolvedIdentity: {
        identityType: 'nocf',
    ...
    {
      id: 'delete_pdf_nocf_target_override_has_empty_cf_even_if_target_stale_cf',
      input: {
        domain: 'deletePdf',
        signalId: 'sig_delete_pdf_stale_cf_nocf_2',
        targetPath: 'drive_pdf_imports/file_2'
      },
      deletePdfTargetOverridesOnly: {
        identityType: 'nocf',
        identityAnchor: 'NOCF_MANUAL_004',
        legacyNoCfCode: 'MANUAL_004',
    ...
    {
      id: 'done_result_omits_identity_existing_nocf_ignores_stale_cf',
      input: {
        domain: 'backup',
        signalId: 'sig_existing_nocf_stale_cf_1',
        identityType: 'nocf',
        identityAnchor: 'NOCF_MANUAL_005',
        legacyNoCfCode: 'MANUAL_005',
        targetFiscalCode: 'RSSMRA80A01H501U',
        targetFullName: 'Amedeo Fantauzzo',
        identityResolutionReasons: ['existing_signal_identity_reason']
      },
    ...
    {
      id: 'done_result_omits_identity_existing_nocf_preserves_result_metadata',
      input: {
        domain: 'backup',
        signalId: 'sig_existing_nocf_result_metadata_1',
        identityType: 'nocf',
        identityAnchor: 'NOCF_MANUAL_006',
        legacyNoCfCode: 'MANUAL_006',
        targetFiscalCode: 'RSSMRA80A01H501U',
        targetFullName: 'Amedeo Fantauzzo',
        identityResolutionReasons: ['existing_signal_identity_reason']
      },
      ...
      expectedDoneResultIdentity: {
        identityType: 'nocf',
        identityAnchor: 'NOCF_MANUAL_006',
        identityAnchorCanonical: true,
        targetFiscalCode: '',
        legacyNoCfCode: 'MANUAL_006',
        resultIdentityType: 'nocf',
        resultIdentityAnchor: 'NOCF_MANUAL_006',
        resultIdentityAnchorCanonical: true,
        resultCf: '',
        resultLegacyNoCfCode: 'MANUAL_006',
        resultReason: 'existing_signal_identity_reason',
        reason: 'existing_signal_identity_reason'
      }
    }
    ,
    {
      id: 'normalize_existing_nocf_with_stale_cf_preserves_nocf_identity',
      input: {
        domain: 'backup',
        signalId: 'sig_existing_nocf_processing_stale_cf_1',
        identityType: 'nocf',
        identityAnchor: 'NOCF_MANUAL_007',
        legacyNoCfCode: 'MANUAL_007',
        targetFiscalCode: 'RSSMRA80A01H501U',
        targetFullName: 'Amedeo Fantauzzo',
        identityResolutionReasons: ['existing_signal_identity_reason']
      },
    ...
    {
      id: 'done_result_omits_cf_existing_cf_anchor_preserved',
      input: {
        domain: 'backup',
        signalId: 'sig_existing_cf_anchor_no_target_cf_1',
        identityType: 'cf',
        identityAnchor: 'RSSMRA80A01H501U',
        targetFiscalCode: '',
        identityResolutionReasons: ['existing_cf_identity_reason']
      },
      doneResult: {
        ok: true,
        domain: 'backup',
      ...
      expectedDoneResultIdentity: {
        ...
        targetFiscalCode: 'RSSMRA80A01H501U',
        legacyNoCfCode: '',
        resultIdentityType: 'cf',
        resultIdentityAnchor: 'RSSMRA80A01H501U',
        resultIdentityAnchorCanonical: true,
        resultCf: 'RSSMRA80A01H501U',
        resultLegacyNoCfCode: '',
        resultReason: 'existing_cf_identity_reason',
        reason: 'existing_cf_identity_reason'
      }
    }
  ...
  var items = cases.map(function (item) {
    var actual;
    if (item.doneResult) {
      actual = buildRuntimeSignalWriteData_(item.input, buildRuntimeSignalDoneWriteOverrides_(item.input, item.doneResult, '2026-01-01T00:00:00.000Z'));
      if (item.expectedDoneResultIdentity) {
        actual = summarizeMigration1RuntimeSignalDoneIdentityForTest_(actual);
      }
    } else if (item.processingOverrides) {
      actual = buildRuntimeSignalWriteData_(item.input, item.processingOverrides);
    } else if (item.deletePdfTarget) {
      actual = resolveMigration1RuntimeSignalIdentity_(item.input, buildMigration1DeletePdfRuntimeSignalIdentityOverrides_(item.input, item.deletePdfTarget, normalizeCf_(item.deletePdfTarget.patientFiscalCode || item.deletePdfTarget.fiscalCode || item.deletePdfTarget.patientCf)));
    } else if (item.deletePdfTargetOverridesOnly) {
      actual = buildMigration1DeletePdfRuntimeSignalIdentityOverrides_(item.input, item.deletePdfTargetOverridesOnly, normalizeCf_(item.deletePdfTargetOverridesOnly.patientFiscalCode || item.deletePdfTargetOverridesOnly.fiscalCode || item.deletePdfTargetOverridesOnly.patientCf));
    } else if (item.identityOverrides) {
      actual = resolveMigration1RuntimeSignalIdentity_(item.input, item.identityOverrides);
    } else if (item.resultInput) {
      actual = sanitizeRuntimeSignalResult_(item.resultInput);
    } else if (item.expectedWriteData) {
      actual = buildRuntimeSignalWriteData_(item.input, {});
    } else {
      actual = resolveMigration1RuntimeSignalIdentity_(item.input, {});
    }
    var expected = item.expected || item.expectedWriteData || item.expectedSanitized || item.expectedDoneWriteData || item.expectedDoneResultIdentity || item.expectedResolvedIdentity || item.expectedOverrides || item.expectedProcessingWriteData || {};
...
function summarizeMigration1RuntimeSignalDoneIdentityForTest_(data) {
  data = data || {};
  var result = data.result || {};
  return {
    identityType: String(data.identityType || '').trim().toLowerCase(),
    identityAnchor: String(data.identityAnchor || '').trim(),
    identityAnchorCanonical: !!data.identityAnchorCanonical,
    targetFiscalCode: normalizeCf_(data.targetFiscalCode || ''),
    legacyNoCfCode: String(data.legacyNoCfCode || '').trim(),
    identityResolutionReasons: uniqueNonEmptyStrings_(data.identityResolutionReasons || []),
    resultIdentityType: String(result.identityType || '').trim().toLowerCase(),
    resultIdentityAnchor: String(result.identityAnchor || '').trim(),
    resultIdentityAnchorCanonical: !!result.identityAnchorCanonical,
    resultCf: normalizeCf_(result.cf || ''),
    resultLegacyNoCfCode: String(result.legacyNoCfCode || '').trim(),
    resultIdentityResolutionReasons: uniqueNonEmptyStrings_(result.identityResolutionReasons || [])
  };
}
...
function compareMigration1RuntimeSignalIdentityExpected_(actual, expected) {
  ...
  Object.keys(expected || {}).forEach(function (key) {
    ...
    if (key === 'resultReason') {
      if ((actual.resultIdentityResolutionReasons || []).indexOf(expected[key]) === -1) {
        mismatches.push('missing_result_reason_' + expected[key]);
      }
      return;
    }
