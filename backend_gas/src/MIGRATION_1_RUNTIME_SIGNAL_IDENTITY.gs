...
function runMigration1RuntimeSignalIdentitySelfTest_() {
  ...
  var items = cases.map(function (item) {
    ...
    var expected = item.expected || item.expectedWriteData || item.expectedSanitized || item.expectedDoneWriteData || item.expectedResolvedIdentity || item.expectedOverrides || {};
