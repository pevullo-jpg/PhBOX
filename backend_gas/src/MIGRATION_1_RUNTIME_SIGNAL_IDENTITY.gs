...
function resolveMigration1RuntimeSignalIdentity_(signal, overrides) {
  ...
  return {
    ...
    identityAnchorCanonical: !!identity.ok,
    targetFiscalCode: targetFiscalCode,
    legacyNoCfCode: String(identity.noCfCode || '').trim(),
    identityResolutionReasons: uniqueNonEmptyStrings_(identity.reasons || []),
    firestoreReads: 0,
    firestoreWrites: 0,
...
function runMigration1RuntimeSignalIdentitySelfTest_() {
  ...
  var items = cases.map(function (item) {
    ...
    var expected = item.expected || item.expectedWriteData || item.expectedSanitized || {};
