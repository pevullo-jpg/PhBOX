  ...
  for (var t = 0; t < threads.length; t++) {
    ...
    for (var m = 0; m < messages.length; m++) {
      ...
      if (saveErrors > 0) {
        ...
        var clearTechnicalThreadState = ensureRuntimeThreadShape_(runtimeIndex.threadsById[threadId]);
        if (clearTechnicalThreadState.status === 'technical_error') {
          clearTechnicalThreadState.status = 'pending';
          clearTechnicalThreadState.terminal = false;
          clearTechnicalThreadState.finalizationStatus = '';
          clearTechnicalThreadState.updatedAt = new Date().toISOString();
          runtimeIndex.threadsById[threadId] = clearTechnicalThreadState;
          addDirtyThreadId_(runtimeIndex, threadId);
        }
      }
      ...
      messageDiagnostic.finalDisposition = saveErrors > 0
        ? (savedAny ? 'partial_technical_error_retry' : 'technical_error_retry')
