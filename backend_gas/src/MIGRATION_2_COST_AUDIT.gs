var PHBOX_M2_COST_VERSION_ = 'M2_COST_v3';
...
function runMigration2CostAuditSelfTest_() {
  var cases = [
    ...
    {
      id: 'write_detected_fails_even_with_zero_budget',
      ...
      expected: { ok: false, reason: 'm2_cost_violation', violation: 'firestore_writes_detected' }
    },
    {
      id: 'positive_write_budget_still_blocks_any_write',
      input: buildMigration2CostAuditRuntimeResult_(buildMigration2CostAuditTestE2eStatus_(true, { e2eVersion: PHBOX_M2_COST_REQUIRED_E2E_VERSION_, firestoreWrites: 1 }), { maxReadsPerRun: 20, maxWritesPerRun: 10, maxReadsPerHour: 240, maxWritesPerHour: 120, runsPerHour: 12 }, []),
      expected: { ok: false, reason: 'm2_cost_violation', violation: 'firestore_writes_detected' }
    },
    {
...
function compareMigration2CostAuditExpected_(result, actual, expected) {
  ...
  if (actual.firestoreWrites > 0 && !(expected && (expected.violation === 'firestore_writes_detected' || expected.violation === 'firestore_writes_per_hour_detected'))) mismatches.push('unexpected_firestore_writes');
