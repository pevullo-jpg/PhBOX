var PHBOX_M2_ROLLBACK_VERSION_ = 'M2_ROLLBACK_v6';
...
function buildMigration2RollbackResult_(data) {
  ...
  var cutonTenantId = String(cutonStats.tenantId || '').trim();
  var cutonCutoverTenantId = String(cutonStats.cutoverTenantId || '').trim();
  var cutonRequiresTenantProof = !!cutonStats.enabled || !!cutonStats.cutoverAuthorized || !!cutonStats.cutover;
  if (cutonRequiresTenantProof && (!cutonTenantId || !cutonCutoverTenantId)) violations.push('m2_cuton_tenant_fields_missing');
  if (cutonRequiresTenantProof && rollbackTenantId && cutonTenantId && rollbackTenantId !== cutonTenantId) violations.push('rollback_tenant_mismatch');
  if (cutonRequiresTenantProof && rollbackTenantId && cutonCutoverTenantId && rollbackTenantId !== cutonCutoverTenantId) violations.push('rollback_tenant_mismatch');
...
function runMigration2RollbackSelfTest_() {
  ...
  var cases = [
    ...
    {
      id: 'cuton_status_missing_tenant_fields_blocks_rollback',
      result: (function () {
        var status = buildMigration2RollbackSyntheticCutonStatus_({ tenantId: tenant, firestoreReads: 4 });
        delete status.stats.tenantId;
        delete status.stats.cutoverTenantId;
        return buildMigration2RollbackResult_({ enabled: true, rollbackTenantId: tenant, cutonStatus: status });
      })(),
      expected: { ok: false, rollbackAuthorized: false, violation: 'm2_cuton_tenant_fields_missing' }
    },
    {
    ...
    {
      id: 'cuton_disabled_ignores_stale_tenant_mismatch',
      result: buildMigration2RollbackResult_({ enabled: true, rollbackTenantId: tenant, cutonStatus: buildMigration2RollbackSyntheticCutonStatus_({ enabled: false, tenantId: 'altra_farmacia', cutoverTenantId: 'altra_farmacia', cutoverAuthorized: false, cutover: false }) }),
      expected: { ok: true, skipped: true, rollbackAuthorized: false, legacyRouteActive: true, cutover: false, reason: 'cuton_already_disabled_legacy_active', violation: '' }
    },
    {
