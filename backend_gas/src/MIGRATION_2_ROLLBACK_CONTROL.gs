var PHBOX_M2_ROLLBACK_VERSION_ = 'M2_ROLLBACK_v5';
...
function buildMigration2RollbackResult_(data) {
  ...
  if (rollbackTenantId && cutonTenantId && rollbackTenantId !== cutonTenantId) violations.push('rollback_tenant_mismatch');
  if (rollbackTenantId && cutonCutoverTenantId && rollbackTenantId !== cutonCutoverTenantId) violations.push('rollback_tenant_mismatch');
