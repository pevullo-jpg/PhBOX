...
function buildMigration3E2eResult_(data) {
  ...
  statsInput.m3FreezeAllowed = violations.length === 0 && statsInput.m3FreezeAllowed;
...
function runMigration3E2eSelfTest_() {
  ...
  function add(id, overrides, expectedOk, expectedViolations) {
    ...
    var freezeFlagCoherent = expectedOk ? result.stats.m3FreezeAllowed === true : result.stats.m3FreezeAllowed === false;
    var passed = result.ok === expectedOk && freezeFlagCoherent && migration3E2eHasExpectedViolations_(result.stats.violations, expectedViolations || []);
