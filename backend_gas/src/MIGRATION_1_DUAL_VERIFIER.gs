...
function runMigration1DualVerifierRuntimeStatus_() {
  ...
  return runMigration1DualVerifierForLegacyPaths_(getPhboxConfig_(), targetRuntime, samplePaths);
}
...
function buildMigration1DualComparableSignature_(doc) {
  return stableStringifyMigration1Dual_(normalizeMigration1DualComparableValue_(doc || {}, true));
}

function normalizeMigration1DualComparableValue_(value, isTopLevel) {
  if (value === null || value === undefined) return null;
  if (Array.isArray(value)) {
    return value.map(function (item) {
      return normalizeMigration1DualComparableValue_(item, false);
    });
  }
  if (typeof value === 'object') {
    ...
    Object.keys(value).sort().forEach(function (key) {
      ...
      out[key] = normalizeMigration1DualComparableValue_(value[key], false);
    });
    return out;
  }
  return value;
...
function runMigration1DualVerifierSelfTest_() {
  ...
  var cases = [
    ...
    {
      id: 'nested_document_path_field_mismatch_detected',
      legacyDoc: { fiscalCode: 'RSSMRA80A01H501U', nested: { documentPath: 'business/path/a' } },
      targetDoc: { fiscalCode: 'RSSMRA80A01H501U', nested: { documentPath: 'business/path/b' } },
      expected: { matched: false, reason: 'signature_mismatch' }
    },
    {
