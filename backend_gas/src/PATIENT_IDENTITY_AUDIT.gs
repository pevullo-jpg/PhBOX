function auditPatientIdentityConsistency(options) {
  ...
  var maxSamples = Math.max(1, Math.min(100, Number(options.maxSamples || 30)));
  ...
  patients.forEach(function (patient) {
    ...
    if (isDocTmp && fieldCf && !isFieldTmp) {
      var tmpWithReal = {
        ...
        targetExists: !!canonicalPatientByCf[fieldCf],
        fullName: auditPatientIdentityReadString_(patient && (patient.fullName || patient.patientFullName))
      };
      auditPatientIdentityPushSample_(samples.tmpWithRealFiscalCode, tmpWithReal, maxSamples);
  ...
  families.forEach(function (family) {
    ...
    if (tmpMembers.length) {
      auditPatientIdentityPushSample_(samples.familiesWithTmpMembers, {
        familyId: familyId,
        tmpMembers: auditPatientIdentityUnique_(tmpMembers)
      }, maxSamples);
    }
    if (duplicates.length) {
      auditPatientIdentityPushSample_(samples.familiesWithDuplicateMembers, {
        familyId: familyId,
        duplicateMembers: auditPatientIdentityUnique_(duplicates)
      }, maxSamples);
    }
  });
  ...
  var result = {
    ...
    counts: {
      ...
      duplicateFiscalCodeGroups: samples.duplicateFiscalCodeGroups.length,
      documentIdFiscalCodeMismatches: samples.documentIdFiscalCodeMismatches.length,
      indexesWithoutCanonicalPatient: samples.indexesWithoutCanonicalPatient.length,
      patientsWithoutIndex: samples.patientsWithoutIndex.length,
      operationalDocsWithoutCanonicalPatient: samples.operationalDocsWithoutCanonicalPatient.length,
      familiesWithTmpMembers: samples.familiesWithTmpMembers.length,
      familiesWithDuplicateMembers: samples.familiesWithDuplicateMembers.length,
      doctorLinksWithTmpOrMissingPatient: samples.doctorLinksWithTmpOrMissingPatient.length,
      therapeuticAdviceWithoutCanonicalPatient: samples.therapeuticAdviceWithoutCanonicalPatient.length,
      mergeCandidates: samples.mergeCandidates.length
    },
