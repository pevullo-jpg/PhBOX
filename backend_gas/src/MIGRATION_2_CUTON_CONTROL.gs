var PHBOX_M2_CUTON_VERSION_ = 'M2_CUTON_v2';
...
function buildMigration2CutonResult_(data) {
  ...
  var cutoverTenantId = String(data.cutoverTenantId || '').trim();
  var violations = [];
  var reason = '';
...
function readMigration2CutonSettings_(props) {
  ...
  return {
    ...
    cutoverTenantId: String(props.getProperty(PHBOX_M2_CUTON_TENANT_ID_PROPERTY_) || '').trim()
  };
}
