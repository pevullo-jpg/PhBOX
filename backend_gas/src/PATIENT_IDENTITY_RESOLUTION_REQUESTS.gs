...
function processMaterializePatientIdentityMerges(options) {
  ...
  var sourceScan = materializeListMergedSourcePatients_(cfg, maxSourceScan);
  var sources = sourceScan.items.slice(0, maxSourceScan);
  var result = {
    ok: true,
...
function materializeListMergedSourcePatients_(cfg, maxSourceScan) {
  var pageSize = 50;
  var scanned = 0;
  ...
  while (scanned < maxSourceScan) {
    var limit = Math.min(pageSize, maxSourceScan - scanned);
    ...
    for (var i = 0; i < page.length; i++) {
      ...
      if (scanned >= maxSourceScan) break;
    }

    if (page.length < limit) break;
  }

  if (scanned >= maxSourceScan) scanLimitReached = true;
  return {
    items: items,
    scanned: scanned,
