...
function processPatientIdentityMergeRequests(options) {
  ...
  var seenMergeKeys = {};
  var pendingCanonicalTargetBySource = identityMergeBuildPendingCanonicalTargetBySource_(requests);
  var result = {
    ok: true,
  ...
  for (var i = 0; i < requests.length; i++) {
    ...
    if (!identityResolutionRequestsIsSafeRealCf_(targetId) && pendingCanonicalTargetId) {
      result.requestsProcessed++;
      ...
      identityResolutionRequestsAddSample_(result.samples.deferred, {
        ...
        reason: 'target_tmp_has_pending_canonical_merge',
