...
Il frontend PhBOX considera valido solo un utente Firebase Authentication email/password con provider `password`, email Firebase normalizzata non vuota ed email provider coerente.
...
La dipendenza `google_sign_in` può restare nel progetto finché esistono servizi legacy che la importano, ma non è usata dal login tenant introdotto da questo gate. Il login tenant usa esclusivamente `FirebaseAuth.signInWithEmailAndPassword`.
