...
Se `tenantId` non è presente o gate shadow è OFF:

```text
nessuna lettura target
nessun effetto operativo
```
...
2. Gate shadow ON + tenantId valido → query bounded tenants/{tenantId}/assistiti.
3. Output legacy invariato con shadow ON.
4. NOCF target resolved_manual letto e contato.
5. CF target letto ma non usato per publish.
6. Nessuna write su tenants/*.
7. Nessuna write su root collection aggiuntiva.
8. Nessun Gmail/Drive/PDF lifecycle call nuovo.
9. Costo dichiarato: max reads/run bounded.
10. Rollback: gate OFF elimina ogni lettura target.
```
...
3. Path target può essere costruito male se tenantId non è canonico.
...
M1-SHADOW: autorizzabile solo come read-only bounded shadow
