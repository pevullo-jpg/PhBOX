...
Fix #250 — correzione contratto operativo.  
...
createdAt: timestamp | string parseable
updatedAt: timestamp | string parseable
...
- `createdAt` e `updatedAt` non sono ammessi null nella copia reale;
- se la sorgente legacy esiste ma non espone timestamp affidabili, il runtime futuro dovrà usare timestamp tecnico controllato di copia;
- se la sorgente legacy è assente, il run deve bloccarsi con 0 write.
...
- `createdAt` o `updatedAt` assenti dopo la normalizzazione preview;
...
- verificare `createdAt` e `updatedAt` non null;
...
10. `createdAt` o `updatedAt` null dopo preview → reject.
11. Post-copia mismatch assistitoId → verifica fallita.
