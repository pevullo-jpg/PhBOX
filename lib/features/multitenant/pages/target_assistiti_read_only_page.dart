...
class _TargetAssistitiReadOnlyPageState extends State<TargetAssistitiReadOnlyPage> {
  ...
  Future<void> _loadNoCfIdentityResolutionPending() async {
    ...
    try {
      ...
      final RealAssistitiNoCfIdentityResolutionPendingItem? autoDialogItem =
          result.singlePendingItemForAutoDialog;
      if (autoDialogItem != null) {
        await _openNoCfIdentityResolutionDialog(autoDialogItem);
      }
    } catch (error) {
      if (!mounted || requestToken != _nocfIdentityRequestSerial) {
        return;
