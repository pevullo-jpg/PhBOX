...
class _IdentityResolutionPageState extends State<IdentityResolutionPage> {
  ...
  Future<void> _searchCandidates() async {
    ...
    try {
      ...
      if (_isLatestSearchToken(requestToken)) {
        setState(() {
          _loading = false;
        });
      }
  ...
  bool _isLatestSearchToken(int requestToken) {
    if (!mounted) return false;
    return requestToken == _searchRequestToken;
  }

  bool _isCurrentSearch(int requestToken, String query) {
    return _isLatestSearchToken(requestToken) &&
