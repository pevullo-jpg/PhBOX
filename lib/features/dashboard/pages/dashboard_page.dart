...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _startDashboardTotalsListener() {
    ...
    _dashboardTotalsSubscription = _dashboardTotalsRepository.watchMainTotals().listen(
      (DashboardTotalsSnapshot? snapshot) async {
        ...
        unawaited(_loadExpiringRecipesSectionIfNeeded(snapshotTotals, snapshot.updatedAt));
      },
  ...
  String _expiringRecipesSignatureForTotals(
  ...
  Future<void> _loadExpiringRecipesSectionIfNeeded(
    ...
    final String signature = _expiringRecipesSignatureForTotals(totals, updatedAt);
    if (!force && (_expiringRecipesLoading || _expiringRecipesSignature == signature)) {
      return;
    }
    ...
    try {
      ...
      final List<DashboardExpiringRecipe> visibleItems =
          await _filterPendingExpiringRecipes(snapshot?.items ?? const <DashboardExpiringRecipe>[]);
      if (!mounted) {
        return;
      }
      setState(() {
  ...
  Future<bool> _confirmDeleteExpiringRecipe(DashboardExpiringRecipe item) async {
    final bool? confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'La ricetta ${item.fileName.trim().isEmpty ? item.importId : item.fileName} verrà rimossa dalla dashboard mantenendo i dati estratti nel database. Continuare?',
  ...
  Future<void> _requestExpiringRecipeDelete(DashboardExpiringRecipe item) async {
    if (!await _confirmDeleteExpiringRecipe(item)) {
      return;
    }
    await _drivePdfImportsRepository.requestPdfDelete(
      item.importId,
      fiscalCode: item.patientFiscalCode,
    );
    await savePendingPdfDelete(
    ...
    setState(() {
      _expiringRecipes = _expiringRecipes
          .where((DashboardExpiringRecipe current) => current.importId != item.importId)
          .toList();
      _expiringRecipesMessage = _expiringRecipes.isEmpty && _dashboardTotals.expiringCount > 0
          ? 'Ricette in scadenza nascoste localmente dopo richiesta eliminazione.'
          : '';
    });
  }
  ...
  List<Widget> _buildFlagChips(_PatientDashboardSummary item) {
    ...
    if (item.hasDpc) {
      final int dpcCount = item.dpcItems.isNotEmpty ? item.dpcItems.length : math.max(1, item.recipeCount);
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          IconButton(
            tooltip: 'Elimina ricetta',
            onPressed: () => unawaited(_requestExpiringRecipeDelete(item)),
