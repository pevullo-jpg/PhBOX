...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _startDashboardTotalsListener() {
    ...
    _dashboardTotalsSubscription = _dashboardTotalsRepository.watchMainTotals().listen(
      (DashboardTotalsSnapshot? snapshot) async {
        ...
        _reconcilePendingArchiveTotalsWithBackend(snapshotTotals);
  ...
  void _reconcilePendingArchiveTotalsWithBackend(_DashboardTotals snapshotTotals) {
    _pendingArchiveRecipeCountDelta = _reconcilePendingArchiveDelta(
      pendingDelta: _pendingArchiveRecipeCountDelta,
      previousBackendCount: _lastBackendArchiveRecipeCount,
      nextBackendCount: snapshotTotals.recipeCount,
    );
    _pendingArchiveDpcCountDelta = _reconcilePendingArchiveDelta(
      pendingDelta: _pendingArchiveDpcCountDelta,
  ...
  int _reconcilePendingArchiveDelta({
    ...
    required int? previousBackendCount,
    required int nextBackendCount,
  }) {
    if (pendingDelta == 0 || previousBackendCount == null) {
      return pendingDelta;
    }
    if (pendingDelta < 0) {
      final int backendDecrease = math.max(0, previousBackendCount - nextBackendCount);
      if (backendDecrease <= 0) {
        return pendingDelta;
      }
      return math.min(0, pendingDelta + backendDecrease);
    }
    final int backendIncrease = math.max(0, nextBackendCount - previousBackendCount);
    if (backendIncrease <= 0) {
      return pendingDelta;
    }
    return math.max(0, pendingDelta - backendIncrease);
  }
  ...
  void _applyDashboardTotalsDeltaLocal({
    ...
    if (recipeCountDelta != 0) {
      _lastBackendArchiveRecipeCount ??=
          _clampDashboardTotalInt(_dashboardTotals.recipeCount - _pendingArchiveRecipeCountDelta);
      _pendingArchiveRecipeCountDelta += recipeCountDelta;
    }
    if (dpcCountDelta != 0) {
      _lastBackendArchiveDpcCount ??=
          _clampDashboardTotalInt(_dashboardTotals.dpcCount - _pendingArchiveDpcCountDelta);
  ...
  void _replaceCachedSummaryLocal(_PatientDashboardSummary nextSummary) {
  ...
  PatientDashboardIndexFlag _indexFlagForDashboardFilter(_DashboardCardFilter filter) {
    switch (filter) {
      case _DashboardCardFilter.ricette:
        return PatientDashboardIndexFlag.recipes;
      case _DashboardCardFilter.dpc:
        return PatientDashboardIndexFlag.dpc;
      case _DashboardCardFilter.debiti:
        return PatientDashboardIndexFlag.debt;
  ...
  bool _matchesActiveIndexFilters(_PatientDashboardSummary item) {
    for (final _DashboardCardFilter filter in _activeCardFilters) {
      switch (filter) {
        case _DashboardCardFilter.ricette:
        ...
        case _DashboardCardFilter.dpc:
          ...
          break;
        case _DashboardCardFilter.debiti:
          if (!item.hasDebt) return false;
          break;
        case _DashboardCardFilter.anticipi:
          if (!item.hasAdvance) return false;
          break;
        case _DashboardCardFilter.prenotazioni:
  ...
  Future<_DashboardIndexLoadResult> _loadActiveFamilyFilterIndexRows() async {
    ...
    final List<PatientDashboardIndex> rowsByFamilyId = await _patientDashboardIndexRepository.getByFamilyIds(
      <String>[familyId],
      maxFamilyIds: 1,
      limitPerFamily: _maxDashboardSearchFamilyMembersPerFamily,
    );
    ...
    if (family == null || family.id.trim().isEmpty) {
      return _DashboardIndexLoadResult(rows: rowsByFamilyId);
    }

    final _DashboardSearchFamilyExpansion familyExpansion =
        _buildDashboardFamilyExpansion(<FamilyGroup>[family]);
    final List<PatientDashboardIndex> rowsByMemberCode = familyExpansion.memberFiscalCodes.isEmpty
  ...
  Future<_DashboardSearchFamilyExpansion> _resolveDashboardSearchFamilyExpansion(
    ...
    for (final String fiscalCode in directFiscalCodes) {
      if (familiesById.length >= _maxDashboardSearchFamilyIds ||
          fallbackLookups >= _maxDashboardSearchFamilyMemberLookups) {
        break;
      }
      ...
      if (!familiesById.containsKey(family.id) && familiesById.length >= _maxDashboardSearchFamilyIds) {
        break;
      }
      familiesById[family.id] = family;
      coveredFiscalCodes.addAll(family.memberFiscalCodes.map(_normalizeFiscalCode));
  ...
  Future<_PatientDashboardSummary> _ensureSummaryDetailsForKey(
    _PatientDashboardSummary summary,
    String key,
  ) async {
    final String cf = summary.patient.fiscalCode.trim().toUpperCase();
    if (cf.isEmpty) {
      return summary;
    }
    ...
    if (needsAdvances) {
      advances = results[cursor++] as List<Advance>;
    }
    if (needsBookings) {
      bookings = results[cursor++] as List<Booking>;
    }
    ...
    final _PatientDashboardSummary next = summary.copyWith(
      ...
      advances: advances,
      bookings: bookings,
      hasDpc: loadedDpcCount == null ? null : loadedDpcCount > 0,
      dpcCount: loadedDpcCount,
      indexedDebtAmount: debts == null ? null : debts.fold<double>(0, (sum, item) => sum + item.residualAmount),
      indexedDebtCount: debts?.length,
      indexedAdvanceCount: advances?.length,
      indexedBookingCount: bookings?.length,
    );
    _replaceCachedSummary(next);
    return next;
  }
  ...
  List<String> _splitBookingEntryInput(String value) {
    return value
        .split(RegExp(r'[\n\r]+'))
        .map((String item) => item.trim())
        .where((String item) => item.isNotEmpty)
  ...
  Future<bool> _addBookingFromDashboard(_PatientDashboardSummary summary) async {
          ...
          child: Column(
            ...
            children: [
              _dialogField(
                ...
                keyboardType: TextInputType.multiline,
                maxLines: 4,
                helperText: 'Per più voci vai a capo. La virgola resta nel testo.',
              ),
              const SizedBox(height: 12),
              _dialogField(
                quantityController,
    ...
    try {
      final List<String> drugNames = _splitBookingEntryInput(drugController.text);
            ...
            try {
              if (key == 'debiti') {
                ...
                try {
                  ...
                  postSaveWarning = ' Override medico non riallineato: $overrideError';
                }
                await _applyDashboardTotalsDelta(advanceCountDelta: savedAdvances.length);
                currentSummary = currentSummary.copyWith(
                  advances: <Advance>[...savedAdvances, ...currentSummary.advances],
                ...
                await _replaceCachedSummaryAwaitingIndex(currentSummary);
                if (postSaveWarning.isNotEmpty) {
                  setState(() {
                    _message = postSaveWarning.trim();
                  });
                }
              } else {
                final List<String> drugNames = _splitBookingEntryInput(bookingDrugController.text);
  ...
  Future<void> _handleFlagTap(_PatientDashboardSummary summary, String key) async {
    final _PatientDashboardSummary detailSummary = await _ensureSummaryDetailsForKey(summary, key);
    if (key == 'ricette') {
      await _openPdfList(detailSummary);
      return;
    }
    if (key == 'dpc') {
      await _openFlagModal(
        title: 'DPC · ${summary.displayName}',
        items: detailSummary.dpcItems.map((item) {
          return _FlagItem(
            title: item.title,
    ...
    if (key == 'quick-edit') {
      ...
      if (selectedKey != null && mounted) {
        await _openEditableFlagModal(summary: detailSummary, key: selectedKey);
      }
      return;
    }
    if (key == 'debiti' || key == 'anticipi' || key == 'prenotazioni') {
      await _openEditableFlagModal(summary: detailSummary, key: key);
      return;
    }
  }
  ...
  void _removeDeletedRecipesFromExpiringSection({
    ...
    setState(() {
      ...
      _dashboardTotals = _DashboardTotals(
        ...
        expiringCount: _clampDashboardTotalInt(_dashboardTotals.expiringCount + expiringCountDelta),
        expiringRecipesSignature: _dashboardTotals.expiringRecipesSignature,
      );
    });
  }
...
bool _resolveDashboardHasExpiryAlertForArchiveState({
  ...
  final bool hasImportExpiry = visibleImports.any((DrivePdfImport item) {
  ...
  final bool hasLegacyExpiry = legacyPrescriptions.any((Prescription item) {
    return _DashboardTotals._isExpiryAlert(
      item.expiryDate ?? item.prescriptionDate.add(const Duration(days: 30)),
    );
  });
  return hasImportExpiry || hasLegacyExpiry;
}
...
class _PatientDashboardSummary {
  ...
  final double indexedDebtAmount;
  final int indexedDebtCount;
  final int indexedAdvanceCount;
  final int indexedBookingCount;
  ...
  const _PatientDashboardSummary({
    ...
    this.familyColorIndex = 0,
    this.indexedDebtAmount = 0,
    this.indexedDebtCount = 0,
    this.indexedAdvanceCount = 0,
    this.indexedBookingCount = 0,
  });
  ...
  _PatientDashboardSummary copyWith({
    ...
    String? familyId,
    String? familyName,
    int? familyColorIndex,
    double? indexedDebtAmount,
    int? indexedDebtCount,
    int? indexedAdvanceCount,
    int? indexedBookingCount,
  }) {
    return _PatientDashboardSummary(
      ...
      familyColorIndex: familyColorIndex ?? this.familyColorIndex,
      indexedDebtAmount: indexedDebtAmount ?? this.indexedDebtAmount,
      indexedDebtCount: indexedDebtCount ?? this.indexedDebtCount,
      indexedAdvanceCount: indexedAdvanceCount ?? this.indexedAdvanceCount,
      indexedBookingCount: indexedBookingCount ?? this.indexedBookingCount,
    );
  }
  ...
  double get totalDebt => debts.isNotEmpty ? debts.fold<double>(0, (sum, item) => sum + item.residualAmount) : indexedDebtAmount;
  int get debtCount => debts.isNotEmpty ? debts.length : indexedDebtCount;
  int get advanceCount => advances.isNotEmpty ? advances.length : indexedAdvanceCount;
  int get bookingCount => bookings.isNotEmpty ? bookings.length : indexedBookingCount;
  bool get hasDebt => totalDebt.abs() > 0.005 || debtCount > 0;
  bool get hasAdvance => advanceCount > 0;
  bool get hasBooking => bookingCount > 0;
  ...
  bool get hasActiveContent => recipeCount > 0 || hasDpc || hasDebt || hasAdvance || hasBooking;
  ...
  factory _PatientDashboardSummary.fromIndex(PatientDashboardIndex item) {
    return _PatientDashboardSummary(
      patient: item.toPatient(),
      doctorName: item.doctorFullName.trim().isEmpty ? '-' : item.doctorFullName.trim(),
      exemptionCode: item.exemptionCode.trim().isEmpty ? '-' : item.exemptionCode.trim(),
      city: item.city.trim().isEmpty ? '-' : item.city.trim(),
      prescriptions: const <Prescription>[],
      imports: const <DrivePdfImport>[],
      debts: const <Debt>[],
      advances: const <Advance>[],
      ...
      familyColorIndex: item.familyColorIndex,
      indexedDebtAmount: item.debtAmount,
      indexedDebtCount: item.debtCount,
      indexedAdvanceCount: item.advanceCount,
      indexedBookingCount: item.bookingCount,
    );
  }
...
class _DashboardFamilyState {
  ...
  factory _DashboardFamilyState.fromSummaries(List<_PatientDashboardSummary> summaries) {
    final counts = <String, int>{};
    final colors = <String, Color>{};
    for (final item in summaries) {
      if (item.familyId.isEmpty) continue;
      if (item.hasActiveContent) {
        counts[item.familyId] = (counts[item.familyId] ?? 0) + 1;
      }
      colors[item.familyId] = FamilyGroupColorUtils.colorForIndex(item.familyColorIndex);
    }
...
class _SummaryCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Color mutedSurface = Color.lerp(AppColors.panel, accent, 0.30)!;
    final Color mutedElevatedSurface = Color.lerp(AppColors.panelElevated, accent, 0.24)!;
    final Color fullSurface = Color.lerp(AppColors.panel, accent, 0.90)!;
    final Color fullElevatedSurface = Color.lerp(AppColors.panelElevated, accent, 0.96)!;
    final Color surface = isSelected ? mutedSurface : fullSurface;
    final Color elevatedSurface = isSelected ? mutedElevatedSurface : fullElevatedSurface;
    final Color textColor = isSelected ? AppColors.textPrimary : const Color(0xFF101010);
    final Color labelColor = isSelected ? AppColors.textSecondary : const Color(0xFF1A1A1A);
    final Color iconBackground = isSelected ? accent.withOpacity(0.38) : Colors.black.withOpacity(0.18);
    ...
    final Widget card = Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: <BoxShadow>[
          BoxShadow(
            color: isSelected ? Colors.black.withOpacity(0.30) : accent.withOpacity(0.22),
            blurRadius: isSelected ? 22 : 24,
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(16),
              border: Border.all(
                color: isSelected ? accent.withOpacity(0.68) : Colors.black.withOpacity(0.18),
              ),
            ),
