...
class PatientsRepository {
  ...
  Future<PatientProfileUpdateResult> mergeTemporaryPatientIntoFiscalCode({
    ...
    final Map<String, dynamic> tombstone = <String, dynamic>{
      'fiscalCode': oldId,
      'fullName': fullName,
      'hiddenFromFrontend': true,
      'patientProfileMigrated': true,
      'migratedToFiscalCode': newId,
      'migrationCompletedAt': isoNow,
      'updatedAt': isoNow,
    };
    batch.set(oldPatientRef, tombstone, SetOptions(merge: true));
  ...
  void _queueManualDoctorLinkMerge({
    ...
    final QueryDocumentSnapshot<Map<String, dynamic>>? oldManual = _firstManualDoctorLink(oldLinks);
    final QueryDocumentSnapshot<Map<String, dynamic>>? targetManual = _firstManualDoctorLink(targetLinks);
  ...
  QueryDocumentSnapshot<Map<String, dynamic>>? _firstManualDoctorLink(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> docs,
  ) {
    for (final QueryDocumentSnapshot<Map<String, dynamic>> doc in docs) {
      final String id = doc.id.trim();
      final String type = _readStringField(doc.data(), 'linkType').toLowerCase();
      if (id.endsWith('__manual') || type == 'manual' || type.isEmpty) return doc;
    }
    return null;
  }
  ...
  void _assertOnlyManualDoctorLinks(
    ...
    for (final QueryDocumentSnapshot<Map<String, dynamic>> doc in docs) {
      final String type = _readStringField(doc.data(), 'linkType').toLowerCase();
      final bool manual = doc.id.endsWith('__manual') || type == 'manual' || type.isEmpty;
      if (!manual) {
        throw PatientProfileUpdateException(
          'Il TMP $temporaryFiscalCode ha link medico backend-owned. Fusione frontend bloccata.',
        );
      }
    }
  }
  ...
  Map<String, dynamic> _buildMergedDashboardIndexMap({
    ...
    final int currentDebtCount = _readIntField(base, 'debtCount');
    final double currentDebtAmount = _readDoubleField(base, 'debtAmount');
    final int currentAdvanceCount = _readIntField(base, 'advanceCount');
    final int currentBookingCount = _readIntField(base, 'bookingCount');
    base['fiscalCode'] = targetFiscalCode;
