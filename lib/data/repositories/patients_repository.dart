...
class PatientsRepository {
  ...
  List<QueryDocumentSnapshot<Map<String, dynamic>>> _manualDoctorLinks(
    ...
    return docs.where((QueryDocumentSnapshot<Map<String, dynamic>> doc) {
      ...
      final String patientFiscalCode = PatientInputNormalizer.normalizeFiscalCode(
        _readStringField(doc.data(), 'patientFiscalCode'),
      );
      final String type = _readStringField(doc.data(), 'linkType').toLowerCase();
      final bool isStandardManualId = id == '${patientFiscalCode}__manual';
      final bool hasManualType = type.isEmpty || type == 'manual';
      return patientFiscalCode.isNotEmpty && isStandardManualId && hasManualType;
  ...
  void _assertOnlyManualDoctorLinks(
    ...
    final String normalizedTemporary = PatientInputNormalizer.normalizeFiscalCode(temporaryFiscalCode);
    for (final QueryDocumentSnapshot<Map<String, dynamic>> doc in docs) {
      final String id = doc.id.trim();
      final String type = _readStringField(doc.data(), 'linkType').toLowerCase();
      final bool isStandardManualId = id == '${normalizedTemporary}__manual';
      final bool hasManualType = type.isEmpty || type == 'manual';
      final bool manual = isStandardManualId && hasManualType;
      if (!manual) {
        throw PatientProfileUpdateException(
          'Il TMP $normalizedTemporary ha link medico non manuale o non standard. Fusione frontend bloccata.',
        );
      }
    }
  }
