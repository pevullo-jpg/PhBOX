...
class RealAssistitiNoCfIdentityResolutionWriter {
    ...
    await firestore.runTransaction<void>((Transaction transaction) async {
      ...
      if (!snapshot.exists || currentData == null) {
        throw const RealAssistitiNoCfIdentityResolutionRejectedException(
          code: 'identity_resolution_document_missing',
          message: 'Assistito target non trovato: risoluzione identità NOCF bloccata.',
        );
      }
      if (!isPendingManualPayload(currentData)) {
        throw const RealAssistitiNoCfIdentityResolutionRejectedException(
          code: 'identity_resolution_not_pending',
          message: 'Assistito target non più pending_manual: risoluzione identità NOCF bloccata.',
        );
      }
      transaction.update(documentRef, patch);
    });
  ...
  static String _normalizeHumanNamePart(String value, {required String label}) {
    ...
    if (_looksFiscalCodeLike(normalized)) {
      throw RealAssistitiNoCfIdentityResolutionRejectedException(
        code: '${label}_fiscal_code_like',
        message: '$label non può essere un codice fiscale.',
      );
    }
    return normalized
        .split(' ')
  ...
  static bool isPendingManualPayload(Map<String, dynamic> rawData) {
    return _readString(rawData['identityResolutionStatus']) == pendingStatus ||
        _readNestedIdentityResolutionStatus(rawData) == pendingStatus ||
        _readString(rawData['nameSplitConfidence']) == pendingConfidence;
  }

  static String _readNestedIdentityResolutionStatus(Map<String, dynamic> rawData) {
  ...
  static bool _looksFiscalCodeLike(String value) {
    final String compact = value.replaceAll(RegExp(r'\s+'), '').trim().toUpperCase();
    return RegExp(r'^[A-Z]{6}[0-9]{2}[A-Z][0-9]{2}[A-Z][0-9]{3}[A-Z]$').hasMatch(compact);
  }
