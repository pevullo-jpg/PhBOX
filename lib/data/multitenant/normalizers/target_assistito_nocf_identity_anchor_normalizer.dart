...
class TargetAssistitoNoCfPromotionResult {
  final String previousCf;
  ...
  const TargetAssistitoNoCfPromotionResult({
    required this.previousCf,
  ...
  bool get isPromotedToCf => identityType == TargetAssistitoNoCfIdentityAnchorNormalizer.identityTypeCf;

  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      'previousCf': previousCf,
      'cf': promotedCf,
      'identityType': identityType,
      'identityAnchor': identityAnchor,
...
class TargetAssistitoNoCfIdentityAnchorNormalizer {
  ...
  static TargetAssistitoNoCfPromotionResult buildNoCfToRealCfPromotion({
    ...
    final String normalizedCurrentCf = currentCf.trim().toUpperCase();
    final String normalizedCurrentIdentityType = currentIdentityType.trim().toLowerCase();
    final String normalizedCurrentIdentityAnchor = currentIdentityAnchor.trim().toUpperCase();
    final String normalizedNewCf = TargetAssistitoIdentityNormalizer.normalizeCf(newRawCf);

    if (normalizedCurrentIdentityType != identityTypeNoCf) {
      throw const TargetAssistitoNoCfIdentityAnchorRejectedException(
        code: 'promotion_source_not_nocf',
        message: 'La promozione a CF reale è ammessa solo per assistiti NOCF.',
      );
    }
    if (!isCanonicalNoCf(normalizedCurrentCf)) {
      throw const TargetAssistitoNoCfIdentityAnchorRejectedException(
        code: 'promotion_source_nocf_not_canonical',
        message: 'Lo pseudo-CF corrente non è un NOCF canonico.',
      );
    }
    if (normalizedCurrentIdentityAnchor != normalizedCurrentCf) {
      throw const TargetAssistitoNoCfIdentityAnchorRejectedException(
        code: 'promotion_identity_anchor_mismatch',
        message: 'identityAnchor corrente non coerente con lo pseudo-CF NOCF.',
      );
    }
    if (!TargetAssistitoIdentityNormalizer.isFiscalCodeLike(normalizedNewCf)) {
      throw const TargetAssistitoNoCfIdentityAnchorRejectedException(
        code: 'promotion_target_cf_invalid',
        message: 'CF reale di destinazione non valido o non canonico.',
      );
    }
    if (normalizedNewCf == normalizedCurrentCf) {
      throw const TargetAssistitoNoCfIdentityAnchorRejectedException(
        code: 'promotion_target_same_as_source',
        message: 'CF reale di destinazione uguale allo pseudo-CF corrente.',
      );
    }

    final List<String> normalizedPreviousAnchors = _normalizePreviousIdentityAnchors(
      previousIdentityAnchors,
    );
    if (!normalizedPreviousAnchors.contains(normalizedCurrentCf)) {
      normalizedPreviousAnchors.add(normalizedCurrentCf);
    }

    return TargetAssistitoNoCfPromotionResult(
      previousCf: normalizedCurrentCf,
      promotedCf: normalizedNewCf,
      identityType: identityTypeCf,
      identityAnchor: normalizedNewCf,
      previousIdentityAnchors: List<String>.unmodifiable(normalizedPreviousAnchors),
  ...
  static List<String> _normalizePreviousIdentityAnchors(
    Iterable<String> previousIdentityAnchors,
  ) {
    final List<String> normalized = <String>[];
    for (final String anchor in previousIdentityAnchors) {
      final String value = anchor.trim().toUpperCase();
      if (value.isEmpty) {
        continue;
      }
      if (_containsUnsafePathSeparator(value)) {
        throw const TargetAssistitoNoCfIdentityAnchorRejectedException(
          code: 'previous_identity_anchor_not_canonical',
          message: 'previousIdentityAnchors contiene un valore non canonico.',
        );
      }
